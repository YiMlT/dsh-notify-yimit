...
const DEFAULT_CONFIG = Object.freeze({
	...
	textColor: '#e6edf3',
	locale: 'en',
	// 通知窗口样式:width 窗口宽度(编辑模式拖拽边缘调整);gap 通知间距(px);
	// position = null 表示默认右下贴边,{left,top} 为任意屏幕位置(工作区相对坐标)。
	toast: {
		width: 340,
		gap: 12,
		position: null,
...
function normalizeConfig(input) {
	...
	out.toast = { width: 340, gap: 12, position: null }
	...
	if (pos !== null && typeof pos === 'object' && !Array.isArray(pos) &&
		...
		out.toast.position = { left: Math.round(pos.left), top: Math.round(pos.top) }
	}
...
export function apply(ctx) {
	...
	function spawnHost() {
		...
		host.stdout.on('data', (d) => {
			...
			while ((nl = buf.indexOf('\n')) !== -1) {
				...
				if (msg.type === 'pos' && typeof msg.key === 'string') {
				...
				} else if (msg.type === 'geometry' && typeof msg.left === 'number' && typeof msg.top === 'number') {
					...
					for (const rec of activeToasts.values()) {
						sendCmd({ cmd: 'size', key: rec.key, width })
					}
					reflowToasts()
				}
			}
		...
		if (pos !== null) {
			...
			for (let i = 0; i < toastOrder.length; i += 1) {
				...
				if (needsTop || needsLeft) {
					const move = { cmd: 'move', key: rec.key, top: newTop }
					if (needsTop) rec.meta.top = newTop
					if (needsLeft) { move.left = pos.left; rec.meta.left = pos.left }
					sendCmd(move)
				}
				prevTop = newTop
			}
	...
	function updateToastTitle(sessionId, title) {
		...
		for (const rec of activeToasts.values()) {
			if (rec.sessionId === sessionId) {
				sendCmd({ cmd: 'title', key: rec.key, title })
			}
		}
	}
		...
		switch (event.type) {
			case 'session/title': {
				if (typeof event.data?.title === 'string' && event.data.title.length > 0) {
					...
					for (let i = queueHead; i < queue.length; i++) {
						if (queue[i].sessionId === id && (queue[i].title.length === 0 || UNTITLED_NAMES.has(queue[i].title)))
	...
	const service = {
		...
		updateConfig(patch) {
			...
			const pToast = patch !== null && typeof patch === 'object' && !Array.isArray(patch) ? patch.toast : undefined
			if (pToast !== null && typeof pToast === 'object' && !Array.isArray(pToast)) {
				if (typeof pToast.width === 'number' && pToast.width !== prev.toast?.width) {
					for (const rec of activeToasts.values()) {
						sendCmd({ cmd: 'size', key: rec.key, width: next.toast.width })
					}
				}
				reflowToasts()
			}
