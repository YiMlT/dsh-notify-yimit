/**
 * dsh-notify 浏览器端 bundle(单文件,经 __ModuleLoader__ 加载,零构建)。
 *
 * 提供两块界面:
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const cssTagId = 'dsh-notify/client.css'
    ...
    function fail(path) {
      throw new Error('dsh-notify: 服务端数据非法 (' + path + ')')
    }
    ...
    const CONTRIBUTION = {
      ...
      descriptors: [
        {
          id: 'dsh-notify#notify/getState', service: 'notify', namespace: 'notify', method: 'getState',
          invocation: { kind: 'direct' }, parameters: [],
          result: { mode: 'strict', typeSymbol: 'dsh-notify#NotifyState', schema: stateCodec },
        },
        {
          id: 'dsh-notify#notify/updateConfig', service: 'notify', namespace: 'notify', method: 'updateConfig',
          invocation: { kind: 'direct' },
          parameters: [{ name: 'patch', wire: 'patch', source: 'json', codec: { mode: 'strict', typeSymbol: 'dsh-notify#NotifyConfigPatch', schema: patchCodec } }],
          result: { mode: 'strict', typeSymbol: 'dsh-notify#NotifyConfig', schema: configCodec },
        },
        {
          id: 'dsh-notify#notify/ackEvents', service: 'notify', namespace: 'notify', method: 'ackEvents',
          invocation: { kind: 'direct' },
          parameters: [{ name: 'ids', wire: 'ids', source: 'json', codec: { mode: 'strict', typeSymbol: 'dsh-notify#EventIds', schema: codecOf((v) => Array.isArray(v) ? v.map((x) => String(x)) : []) } }],
          result: { mode: 'strict', typeSymbol: 'dsh-notify#AckResult', schema: ackCodec },
        },
        {
          id: 'dsh-notify#notify/listBrowsers', service: 'notify', namespace: 'notify', method: 'listBrowsers',
          invocation: { kind: 'direct' }, parameters: [],
          result: { mode: 'strict', typeSymbol: 'dsh-notify#BrowserList', schema: browsersCodec },
        },
      ],
    }
    ...
    async function apply(ctx) {
      ...
      ctx.effect(() => () => { unmount() }, 'dsh-notify: remote contribution')
      ...
      void reload()
      void loadBrowsers()
      ctx.effect(() => ctx.on('connection/reset', () => { void reload(); void loadBrowsers() }), 'dsh-notify: reconnect reload')
      // system 模式走浏览器通知:250ms 轮询把平均延迟从 ~500ms 压到 ~125ms,感知为"即时"。
      const pollTimer = setInterval(() => { void reload() }, 250)
      ctx.effect(() => () => { clearInterval(pollTimer) }, 'dsh-notify: poll timer')
      const onVisible = () => { if (document.visibilityState === 'visible') void reload() }
      document.addEventListener('visibilitychange', onVisible)
      ctx.effect(() => () => { document.removeEventListener('visibilitychange', onVisible) }, 'dsh-notify: visibility reload')

      const sessions = ctx.get('sessions')
      const slots = ctx.get('slots')
      if (slots === undefined) return

      // 会话深链接(桌面浮窗"跳转会话"通道):监听 #dsh-notify/session=<id>。
      const openFromHash = () => {
        try {
          const m = /#dsh-notify\/session=([^&]+)/.exec(window.location.hash)
      ...
      openFromHash()
      ctx.effect(() => () => window.removeEventListener('hashchange', openFromHash), 'dsh-notify: hash listener')
