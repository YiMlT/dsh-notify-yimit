/**
 * dsh-notify-yimit 浏览器端 bundle。
 * 语言跟随：接入 `locale` 服务（由 dsh 框架注入），完全对齐 dsh-market 的标准用法；
 * 旧宿主无该服务时降级到 navigator/localStorage 轮询，绝不阻塞启动。
 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// 选中态用主题主色 + 反色文字:浅色模式下与背景对比也足够清晰(此前浅色下
			// 白底+深灰与容器太接近,深色正常)。
			'.dn-seg-btn[data-on="true"]{background:var(--dsw-alias-state-business-primary);color:var(--dsw-alias-label-primary-inverted);box-shadow:var(--dsw-shadow-lv1)}',
			'.dn-seg-btn[data-on="true"]:hover{background:var(--dsw-alias-state-business-primary);color:var(--dsw-alias-label-primary-inverted)}',
			...
			// 每类型的背景/文字/开关项右对齐(预览块留在左侧)
			'.dn-kind-row>.dn-kind-field:first-of-type{margin-left:auto}',
			// 样式预览:纯色块内嵌类型文字,始终用配置的背景/文字色(内联样式,不受主题影响)
			'.dn-kind-preview{flex:none;min-width:72px;text-align:center;font-size:12px;font-weight:600;line-height:22px;padding:2px 10px;border-radius:6px;white-space:nowrap;box-shadow:inset 0 0 0 1px rgba(0,0,0,.08)}',
			...
			// 浏览器选择器:原生 select 的展开列表为系统绘制、无法贴合主题,改为自绘下拉,
			// 触发器与弹层都走 --dsw-* 变量,深浅色模式下与设置页风格一致。
			'.dn-bselect{position:relative;width:280px;max-width:100%;flex:none}',
			'.dn-bselect-trigger{display:flex;align-items:center;justify-content:space-between;gap:8px;width:100%;font:inherit;font-size:13px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-base);border:1px solid var(--dsw-alias-border-l1);border-radius:8px;padding:5px 10px;cursor:pointer;outline:none}',
			'.dn-bselect-trigger:hover:not(:disabled){border-color:var(--dsw-alias-border-l2)}',
			'.dn-bselect-trigger.open{border-color:var(--dsw-alias-state-business-primary)}',
			'.dn-bselect-trigger:disabled{opacity:.45;cursor:not-allowed}',
			'.dn-bselect-value{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
			'.dn-bselect-arrow{flex:none;font-size:10px;color:var(--dsw-alias-label-tertiary)}',
			'.dn-bselect-pop{position:absolute;top:calc(100% + 4px);left:0;right:0;z-index:100;background:var(--dsw-alias-bg-base);border:1px solid var(--dsw-alias-border-l1);border-radius:10px;box-shadow:var(--dsw-shadow-lv3);padding:4px;max-height:240px;overflow:auto}',
			'.dn-bselect-opt{display:block;width:100%;text-align:left;font:inherit;font-size:13px;color:var(--dsw-alias-label-primary);background:transparent;border:0;border-radius:7px;padding:6px 10px;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
			'.dn-bselect-opt:hover{background:var(--dsw-alias-interactive-bg-hover)}',
			'.dn-bselect-opt.sel{color:var(--dsw-alias-state-business-primary);font-weight:600}',
		...
		const zh = {
			...
			toastStyleTitle: '通知窗口样式',
			toastGapLabel: '通知间距(px)',
			enterEdit: '进入编辑模式',
			endEdit: '完成编辑',
			editHint: '拖动样板窗调整位置;拖拽窗口边缘调整宽度;完成后点击「完成编辑」保存。',
		...
		const en = {
			...
			toastStyleTitle: 'Toast window style',
			toastGapLabel: 'Toast gap (px)',
			enterEdit: 'Enter edit mode',
			endEdit: 'Finish editing',
			editHint: 'Drag the sample window to reposition; drag its edges to resize the width; click "Finish editing" to save.',
		...
		// ===== 兼容层：ctx.locale 可用时直接用，不可用时降级为外部 store =====
		let __localeService = null
		let __fallbackUnsub = null
		const __fallbackListeners = new Set()
		let __fallbackLocale = 'en'

		function detectLocaleId() {
			try {
				if (typeof localStorage !== 'undefined') {
					const sl = localStorage.getItem('dsh-language') || localStorage.getItem('language') ||
						localStorage.getItem('i18nextLng') || localStorage.getItem('locale')
					if (sl) return String(sl).toLowerCase().startsWith('zh') ? 'zh' : 'en'
				}
			} catch {}
			if (typeof document !== 'undefined' && document.documentElement && typeof document
				.documentElement.lang === 'string' && document.documentElement.lang) {
		...
		// 外部 store：供 useSyncExternalStore 使用，统一走 ctx.locale 或降级源
		const localeStore = {
			subscribe(cb) {
				if (__localeService && typeof __localeService.subscribe === 'function') {
					return __localeService.subscribe(cb)
				}
				__fallbackListeners.add(cb)
				if (__fallbackUnsub === null) {
					__fallbackLocale = detectLocaleId()
					__fallbackUnsub = setInterval(() => {
						const next = detectLocaleId()
						if (next !== __fallbackLocale) {
							__fallbackLocale = next
							for (const fn of [...__fallbackListeners]) fn()
						}
					}, 500)
				}
				return () => {
					__fallbackListeners.delete(cb)
					if (__fallbackListeners.size === 0 && __fallbackUnsub !== null) {
						clearInterval(__fallbackUnsub);
						__fallbackUnsub = null
					}
				}
			},
			getSnapshot() {
				if (__localeService && typeof __localeService.getSnapshot === 'function') {
					return __localeService.getSnapshot()
				}
				return {
					active: __fallbackLocale,
					locales: ['zh', 'en'],
					revision: 0
				}
			}
		...
		function currentLangId() {
			try {
				const snap = localeStore.getSnapshot()
				const active = String(snap?.active ?? snap?.locale ?? snap?.language ?? __fallbackLocale ??
					'en')
		...
		function makeT() {
			const dict = () => currentLangId() === 'zh' ? zh : en
			return (key, vars) => {
				const d = dict()
				let text = d[key] ?? en[key] ?? key
				if (vars)
		...
		function parseConfig(v, path) {
			...
			return {
				...
				toast: {
					width: typeof v.toast?.width === 'number' ? v.toast.width : 340,
					gap: typeof v.toast?.gap === 'number' ? v.toast.gap : 12,
					position: v.toast !== null && typeof v.toast === 'object' && v.toast.position !== null &&
						typeof v.toast.position === 'object' && !Array.isArray(v.toast.position) &&
						typeof v.toast.position.left === 'number' && typeof v.toast.position.top === 'number'
						? { left: v.toast.position.left, top: v.toast.position.top }
						: null,
				},
				colors: {
		...
		/** 跳转浏览器选择器:原生 select 的展开列表无法贴合主题,自绘下拉(触发器 + 弹层)。 */
		function BrowserSelect(props) {
			const {
				value,
				options,
				disabled,
			...
			const current = options.find((b) => b.path === value)
			const pick = (path) => {
				onChange(path)
				setOpen(false)
			}
			return el('div', {
					className: 'dn-bselect',
					ref: wrapRef
				},
				el('button', {
					type: 'button',
					className: 'dn-bselect-trigger' + (open ? ' open' : ''),
					disabled: disabled === true,
					'aria-haspopup': 'listbox',
					'aria-expanded': String(open),
					onClick: () => {
						if (disabled !== true) setOpen(!open)
					},
				},
					el('span', {
						className: 'dn-bselect-value'
					}, current !== undefined ? current.name : t('browserDefault')),
					el('span', {
						className: 'dn-bselect-arrow'
					}, open ? '▴' : '▾')),
				open ? el('div', {
						className: 'dn-bselect-pop',
						role: 'listbox'
					},
					el('button', {
						type: 'button',
						...
						className: 'dn-bselect-opt' + (value === '' ? ' sel' : ''),
						onClick: () => pick(''),
					}, t('browserDefault')),
					options.map((b) => el('button', {
						type: 'button',
						role: 'option',
						key: b.id,
						'aria-selected': String(value === b.path),
						className: 'dn-bselect-opt' + (value === b.path ? ' sel' : ''),
						onClick: () => pick(b.path),
					}, b.name)))
				: null)
		}
		...
		function NotifySection(props) {
			...
			// 最新值镜像:卸载清理时读取(避免空依赖闭包拿到陈旧值)。
			const editingRef = useRef(false)
			editingRef.current = editing
			useEffect(() => () => {
				// 离开通知设置页(组件卸载)时若仍在编辑模式,自动关闭样板窗。
				if (editingRef.current) void api.endEditMode()
			}, [])
			...
			const kindRows = KIND_ORDER.map((kind) => {
				...
				return el('div', {
					...
					// 样式预览:类型文字显示在配置色的纯色块内(内联样式,不受深色模式影响)
					el('span', {
						className: 'dn-kind-preview',
						style: {
							background: bg,
							color: fg
						}
						...
						el('div', {
							...
							el('div', {
								...
								el('input', {
									...
									max: 60,
									className: 'dn-input',
									value: config.toast?.gap ?? 12,
									disabled: controlsDisabled,
									onChange: (e) => {
										const g = Math.max(0, Math.min(60, Math.floor(Number(e
											.target.value) || 0)))
										save({
											toast: {
												...(config.toast ?? {}),
							...
							el('div', {
								...
								el('button', {
									...
									onClick: () => {
										if (editing) {
											void api.endEditMode()
											setEditing(false)
										} else {
											setEditing(true)
											void api.startEditMode()
										}
									}
								}, editing ? t('endEdit') : t('enterEdit')),
								el('button', {
									className: 'dn-btn',
									disabled: controlsDisabled || !config.toast?.position,
									onClick: () => {
										save({
											toast: {
												...(config.toast ?? {}),
										...
										// 编辑模式下:重开样板窗,让其在默认位置(右下)刷新显示。
										if (editing) void api.startEditMode()
									}
								}, t('resetPosition'))),
							el('p', {
								className: 'dn-note'
		...
		async function apply(ctx) {
			...
			// 定义 loadBrowsers 和 reload（不依赖 api）
			...
			// ===== 先定义 api（包含 updateConfig），供后续语言同步使用 =====
			const api = {
				reload,
				...
				startEditMode: async () => {
					try {
						await notify.startEditMode()
					} catch {
						/* 静默 */ }
				},
				endEditMode: async () => {
					try {
						await notify.endEditMode()
					} catch {
						/* 静默 */ }
				},
			}

			// ===== 现在设置 __localeService 并使用 api =====
			__localeService = ctx.locale

			if (__localeService && typeof __localeService.register === 'function') {
				ctx.effect(() => __localeService.register(NS, {
					zh,
					en
				}), 'dsh-notify-yimit: dictionaries')
			}

			if (__localeService && typeof __localeService.subscribe === 'function') {
				const syncToHost = () => {
					const newLang = currentLangId()
				...
				const unsub = __localeService.subscribe(syncToHost)
				ctx.effect(() => unsub, 'dsh-notify-yimit: locale sync')
				void syncToHost()
			}
