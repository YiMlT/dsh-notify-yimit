 ...
 *  1. 持久化插件配置($DSH_HOME/storages/dsh-notify-yimit/config.json):总开关、通知方式
...
/** 配置存储:$DSH_HOME/storages/dsh-notify-yimit/config.json(临时文件 + 原子重命名,写防抖)。 */
function openConfig() {
  ...
  function spawnHost() {
    ...
    host.stdout.on('data', (d) => {
      ...
      while ((nl = buf.indexOf('\n')) !== -1) {
        ...
        let msg
        try { msg = JSON.parse(line) } catch { console.warn(`[dsh-notify-yimit] 浮窗回传解析失败: ${line}`); continue }
