 ...
 * 优化点：设置保存防抖、不可见时暂停轮询、降级备份机制、CSS兼容性补丁。
 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// 优化：增加 rgba fallback 防止不支持 color-mix 的浏览器显示异常
		...
		function NotifyOverlay(props) {
			...
			useEffect(() => {
				...
				// 优化：custom 模式下如果事件到达客户端，说明宿主通知失败，降级尝试系统通知
