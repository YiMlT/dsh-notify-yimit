...
const _state$codec = { mode: 'strict', typeSymbol: 'dsh-notify-yimit#NotifyState', schema: getStateSchema }
const _config$codec = { mode: 'strict', typeSymbol: 'dsh-notify-yimit#NotifyConfig', schema: configSchema }
const _patch$codec = { mode: 'strict', typeSymbol: 'dsh-notify-yimit#NotifyConfigPatch', schema: patchSchema }
const _ack$codec = { mode: 'strict', typeSymbol: 'dsh-notify-yimit#AckResult', schema: ackSchema }
const _browsers$codec = { mode: 'strict', typeSymbol: 'dsh-notify-yimit#BrowserList', schema: listBrowsersSchema }
...
export const TYPERT = {
  ...
  invocations: [
    ...
    },
    {
      ...
      invocation: { kind: 'direct' },
      parameters: [
        { name: 'ids', wire: 'ids', source: 'json', codec: { mode: 'strict', typeSymbol: 'dsh-notify-yimit#EventIds', schema: z.array(z.string()) } },
      ],
      result: _ack$codec,
    },
    {
  ...
  model: {
    services: [
      {
        description: 'dsh-notify-yimit 通知服务(ctx.notify):插件配置与通知事件队列。Notification config and event queue service (ctx.notify).',
        summary: 'dsh-notify-yimit 通知配置与事件队列服务 (dsh-notify-yimit notification config & event queue service)。',
        tags: [],
        jsDoc: '/** dsh-notify-yimit 通知配置与事件队列服务(ctx.notify)。dsh-notify-yimit notification config & event queue service (ctx.notify). */',
        key: 'notify',
